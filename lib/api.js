		...
		return rp(options)
			.then(body => {
				...
				return {
					from,
					to,
					date: moment(results.binisTarih).format('l'),
					trDate: moment(results.binisTarih).format('DD/MM/YYYY'),
					remainingSeats,
			...
			.catch(err => {
				console.error(err);
				return { from, to, date: date.add(1, 'days').format('l'), trDate: date.add(1, 'days').format('DD/MM/YYYY'), remainingSeats: -999, availableWagonNumbers: [] };
			});
	}
};
